...
def run_sec_download_for_company(
    ...
    End-to-end pipeline:
      1) Validate company exists
      2) Download SEC filings (ticker or CIK)
      3) Parse and extract AI-relevant sections
      4) Deduplicate at document and section level
      5) Upload to S3
      6) Store document metadata and sections in Snowflake
      7) Return summary
    """
    ...
    try:
        ...
        for idx, f in enumerate(downloaded, 1):
            ...
            # ============================================
            # S3 UPLOAD PHASE (Only non-duplicates)
            # ============================================
            s3_key = (
                f"sec/{ticker}/"
                f"{f.filing_type}/"
            ...
            if parsed.sections:
                ...
                for section_name, section_content in parsed.sections.items():
                    ...
                    if cur.fetchone():
                        ...
                        logger.info(
                            "section_duplicate_skipped",
                            ticker=ticker,
                            filing_type=f.filing_type,
                            section_name=section_name,
                            section_hash=section_hash[:16] + "...",
                            section_word_count=section_word_count,
                        )
                    else:
                        # Section is unique - prepare for storage
                        section_data.append((
                            str(uuid4()),          # id
                            None,                  # document_id (placeholder)
                            section_idx,           # chunk_index (using as section index)
                            section_content,       # ACTUAL SECTION TEXT
                            section_hash,          # Hash of THIS section
                            section_word_count,    # Word count of section
                            None,                  # created_at (placeholder)
                        ))
                        sections_stored += 1
                        
                        logger.info(
                            "section_prepared_for_storage",
                            ticker=ticker,
                            filing_type=f.filing_type,
                            section_name=section_name,
                            section_word_count=section_word_count,
            ...
            # ============================================
            # INSERT DOCUMENT METADATA WITH SECTION STATS
            # ============================================
            doc_id = str(uuid4())
            now = datetime.now(timezone.utc)
            ...
            # ============================================
            # INSERT SECTIONS (NO CHUNKING)
            # ============================================
            if section_data:
                # Update document_id and created_at in prepared sections
                final_sections = [
                    (sec[0], doc_id, sec[2], sec[3], sec[4], sec[5], now)
                ...
                try:
                    ...
                    logger.info(
                        ...
                        ticker=ticker,
                        filing_type=f.filing_type,
                        sections_stored=len(final_sections),
                        total_section_words=sum(sec[5] for sec in final_sections),
                    )
                except Exception as e:
                    logger.error(
                        "section_storage_failed",
                        document_id=doc_id,
                        ticker=ticker,
                        filing_type=f.filing_type,
                        error=str(e),
